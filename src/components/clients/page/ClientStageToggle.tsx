  ...
  const handleStageChange = async (newStage: BirthStage) => {
    ...
    try {
      ...
      if (newStage === 'active-labor' && !client.laborStartTime) {
        ...
        if (!client.deliveryTime) {
          updates.deliveryTime = currentTime;
        }
        if (!client.deliveryDate) {
          updates.deliveryDate = currentTime;
        }
        if (!client.postpartumDate) {
          updates.postpartumDate = currentTime;
        }
        // Also update pregnancyStatus for backward compatibility
        updates.pregnancyStatus = 'postpartum';
